...
export const actions = {
    async select_product({commit},{payload}){
        ...
        var productname = await this.$axios.$get(`http://api:5000/product/get_product?pro_data=${payload.product}`);
        console.log('APIから戻ってきた');
        console.log(productname);
        /* for(var i=0 ; i<productname.length ; i++ ){        
            productname[i].tags = await this.$axios.$get(`http://api:5000/product/get_product?pro_data=${productname[i].product_id}`);//product_idでタグを持ってくるAPI
        } */
    ...
    async getproductdetails({commit},{p_data}){
        ...
        const product_details = await this.$axios.$get(`http://api:5000/product/get_details?id_data=${p_data.product_id}`);
