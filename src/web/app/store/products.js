...
export const actions = {
    async select_product({commit},{payload}){
        ...
        var productname = await this.$axios.$get(`http://express-service/product/get_product?pro_data=${payload.product}`);
    ...
    async getproductdetails({commit},{p_data}){
        ...
        const product_details = await this.$axios.$get(`http://express-service/product/get_details?id_data=${p_data.product_id}`);
    ...
    async products_list({commit},{random_id}){
        ...
        for(var c=0; c<random_id.length; c++){
            ...
            console.log(shop_id)
            var product_data = await this.$axios.$get(`http://express-service/product/random_shop_products?shop_id=${shop_id}`);
