...
export const actions = {
    async search_workshop({commit},{payload}){
        ...
        const shopdata = await this.$axios.$get(`http://express-service/workshop/get_shop?shop_data=${payload.work_shop}`);
    ...
    async random_mall({commit},{random_id}){
        ...
        for(var i=0; i<random_id.length; i++){
            ...
            var randomshop_data = await this.$axios.$get(`http://express-service/workshop/random_shop?shop_id=${shop_id}`);
    ...
    async shop_count({commit}){
        ...
        let count_shop = await this.$axios.$get(`http://express-service/workshop/count_shop`);
    ...
    async get_workshop({commit},{shop_data}){
        ...
        console.log(shop_id)
        const workshopdata = await this.$axios.$get(`http://express-service/workshop/get_workshop_data?shop_id=${shop_id}`);
