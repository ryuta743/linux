...
export const actions = {
    async search_workshop({
        ...
        const shopdata = await this.$axios.$get(`/api-ws/workshop/get_shop?shop_data=${payload.work_shop}`);
    ...
    async random_mall({
        ...
        for (var i = 0; i < random_id.length; i++) {
            ...
            var randomshop_data = await this.$axios.$get(`/api-ws/workshop/random_shop?shop_id=${shop_id}`);
    ...
    async shop_count({
        ...
        let count_shop = await this.$axios.$get(`/api-ws/workshop/count_shop`);
    ...
    async get_workshop({
        ...
        console.log(shop_id)
        const workshopdata = await this.$axios.$get(`/api-ws/workshop/get_workshop_data?shop_id=${shop_id}`);
    ...
    async get_favoshop({
        ...
        var favo_shop = await this.$axios.$get(`/api-ws/workshop/get_favoshop?user_id=${user_id}`);
        var favo_shops = [];
    ...
    async add_favoshop({
        ...
        const result = await this.$axios.$get(`/api-ws/workshop/add_favoshop?user_id=${payload.user_id}&shop_id=${payload.shop_id}`);
    ...
    async del_favoshop({
        ...
        const result = await this.$axios.$get(`/api-ws/workshop/del_favoshop?user_id=${payload.user_id}&shop_id=${payload.shop_id}`);
