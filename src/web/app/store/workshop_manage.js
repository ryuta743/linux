...
export const actions = {
    async getShopdata({
        ...
        try {
            const ws_inf = await this.$axios.$get(`http://express/workshopManage/getShopdata?shop_id=${wsid}`);
    ...
    async getOrderlist({
        ...
        try {
            const ws_inf = await this.$axios.$get(`http://express/workshopManage/getOrderlist?shop_id=${wsid}`);
    ...
    async getOrderdetail({
        ...
        try {
            console.log('aa')
            const ws_inf = await this.$axios.$get(`http://express/workshopManage/getOrderdetail?shop_id=${wsid}&order_number=${order_number}`);
            if (ws_inf.length > 0) {
    ...
    async getProduct({
        ...
        console.log('受け取ったデータ:' + wsid)
        try {
            var products = await this.$axios.$get(`http://express/workshopManage/getProducts?shop_id=${wsid}`);
    ...
    async getSale({
        ...
        console.log('受け取ったデータ:' + wsid)
        try {
            const sales = await this.$axios.$get(`http://express/workshopManage/getSale?shop_id=${wsid}`);
    ...
    async getSaleitem({
        ...
        try {
            console.log(saleid)
            var saleitem = await this.$axios.$get(`http://express/workshopManage/getSaleitem?sale_id=${saleid}`)
        } catch (error) {
    ...
    async addSale({
        ...
        try {
            for (var i = 0; i < items.length; i++) {
                this.$axios.$get(`http://express/workshopManage/addSale?sale_id=${saleid}&product_id=${products[items[i]].product_id}&shop_id=${wsid}&sale_name=${salename}&rate=${rate}`);
            }
            return 'success'
        } catch (error) {
            throw new Error("Error!")
    ...
    async addProduct({
        ...
        try {
            ...
            this.$axios.$get(`http://express/workshopManage/addProduct?shop_id=${payload.wsid}&product_name=${payload.product_name}&product_name_en=${payload.product_name_en}&product_number=${payload.product_number}&price=${payload.price}&record_date=${now}&product_detail=${payload.description}&product_img=${payload.img}&stock=${payload.stock}&safety=${payload.safety}&size=${payload.size}&mate=${payload.mate}&weight=${payload.weight}`)
        } catch (error) {
            throw new Error("Error")
        }
    }
