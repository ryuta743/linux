...
export const actions = {
    ...
    async insert_buy({
        ...
        for (var a = 0; a < kind.length; a++) {
            ...
            var cart_buy = await this.$axios.$get(`/api2/buy/cart_buy?order_id=${saleid}&shop_id=${shop_id}&now=${now}&country=${country}&postalcode=${postalcode}&address=${address}&total=${total}&user_id=${user_id}&user_name=${user_name}&buy_type=${buy_type}&mail=${mail}&tel=${tel}`);
        ...
        for (var i = 0; i < buydata[1].cartdata.length; i++) {
            ...
            var buy_detail = await this.$axios.$get(`/api2/buy/buy_details?order_id=${saleid}&shop_id=${shop_id}&user_id=${user_id}&user_name=${user_name}&product_id=${product_id}&product_name=${product_name}&price=${price}&count=${count}`);
        ...
        for (var z = 0; z < buydata[1].cartdata.length; z++) {
            ...
            var now_count = await this.$axios.$get(`/api2/product/now_count?p_id=${p_id}`);
            console.log(now_count)
            var new_count = now_count[0].stock - buydata[1].cartdata[z].count
            console.log(new_count)
            var upd_count = await this.$axios.$get(`/api2/product/upd_count?new_count=${new_count}&p_id=${p_id}`);
            console.log(upd_count)
        }

        // カート内データを消す処理
        const del_cart = await this.$axios.$get(`/api2/cart/delete_cart?user_id=${buydata[2].user_data.user_data.user_id}`);
