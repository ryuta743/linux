  ...
  <v-app style="background-color: #fff;background-image: url(/wasi.png);">
    ...
    >
      <v-card style="padding: 20px 25px 30px 25px;">
        ...
        <form @submit.prevent>
          <v-text-field name="name" outlined label="mailaddress" v-model="login_mail" id="text_field" style="padding: 0;"></v-text-field>
          <v-text-field name="password" outlined label="password" type="password" v-model="login_password" id="text_field"></v-text-field>
          <div id="login_button">
            <v-btn color="success" type="submit" @click="logindataReq" depressed id="suc_btn">login</v-btn>
            <v-btn color="grey" dark @click="loginDialog = false" depressed id="can_btn">cancel</v-btn>
          </div>
          <div id="chan_pass">
            <nuxt-link to="/change_pass" id="change_pass">forget password</nuxt-link>
          </div>
        </form>
      </v-card>
    </v-dialog>
    ...
    >
      <v-card>
        ...
        <v-card-text style="text-align: center;padding-bottom: 50px;">Done</v-card-text>
        <v-card-actions>
    ...
    >
      <v-card>
        ...
        <v-card-text style="text-align: center;padding-bottom: 50px;">Logout?</v-card-text>
        <v-card-actions>
    ...
    >
      <v-stepper v-model="e1">
        <v-stepper-header>
          <v-stepper-step :complete="e1 > 1" step="1">Your mailaddress</v-stepper-step>

          <v-divider></v-divider>

          <v-stepper-step :complete="e1 > 2" step="2">Other info</v-stepper-step>

          <v-divider></v-divider>

          <v-stepper-step step="3" :complete="e1 > 3">Check your info</v-stepper-step>

          <v-divider></v-divider>

          <v-stepper-step step="4" :complete="e1 > 4">Done</v-stepper-step>
        </v-stepper-header>
        ...
        <v-stepper-items>
          <v-stepper-content step="1">
            <v-layout row wrap justify-center>
              <v-flex md10>
                <v-subheader>mailaddress</v-subheader>
            ...
            <v-layout row wrap justify-end>
              <v-btn color="primary" @click="e1check">check</v-btn>

              <v-btn text @click="createADialog = false">cancel</v-btn>
            </v-layout>
          </v-stepper-content>
          ...
          <v-stepper-content step="2" scrollable>
            <v-layout row wrap justify-center scrollable>
              <v-flex md10 xs8>
                ...
                <v-text-field label placeholder="Taro Tenshoku" outlined v-model="user_name"></v-text-field>
              </v-flex>
              ...
              <v-flex md10 xs8>
                ...
                <v-select outlined :items="countrys" label="your from" item-text="label" item-value="value" v-model="country"></v-select>
              </v-flex>
              <v-flex md11 xs9>
              ...
              <v-flex md10 xs8>
                <v-subheader>skip this step</v-subheader>
                <v-layout row wrap align-center>
                  <v-checkbox label="日本伝統工芸品関係者です" true-value="1" false-value="0"  v-model="craft" disabled></v-checkbox>
                  <v-layout row wrap justify-end>
                    <v-btn color="primary" @click="e1check2">check</v-btn>
                  </v-layout>
                </v-layout>
              </v-flex>
            </v-layout>
          ...
          <v-stepper-content step="3">
            <v-subheader>register this info?</v-subheader>
            <v-simple-table>
              <tbody>
                <tr>
                  <td>mailaddress</td>
              ...
              <v-flex md10>
                <v-card flat>
                  ...
                  <v-card-text style="text-align: center;font-size: 20px;">
                    ...
                    <br />send mail!please check that.
                  </v-card-text>
                  <v-card-text style="text-align: center;">
      ...
      <div v-if="loginuserdata !== null ? true:false" class="no-print">
        <v-menu offset-y class="no-print">
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" text>
              <v-icon color="primary">mdi-account</v-icon>Account
            </v-btn>
          </template>
          <v-list>
    ...
    <div id="subnav" class="no-print">
      <div class="sub_item no-print" @click="$router.push('/en/')"><v-icon color="green darken-2">mdi-home</v-icon>TOP</div>
      <div class="sub_item no-print" @click="$router.push('/en/shop')"><v-icon color="orange darken-2">mdi-gift</v-icon>SHOP</div>
      <div class="sub_item no-print" @click="$router.push('/en/mall')"><v-icon color="purple darken-2">mdi-shopping-search</v-icon>GO TO CITY</div>
      <div class="sub_item no-print" @click="$router.push('/en/contact')"><v-icon color="teal darken-2">mdi-email</v-icon>CONTACT</div>
    </div>
      ...
      <div id="ter">
        ...
        <div id="footer_nav">
          ...
          <div id="navi">
            ...
            <p>privacy</p>
            <p>help</p>
            <nuxt-link to="/contact" id="footer_link">contact</nuxt-link>
          </div>
        </div>
      </div>
      <div id="copy">
